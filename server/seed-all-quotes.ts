...
export async function seedAllQuotes() {
  try {
    console.log('Seeding all 31 quotes...');
    const storage = await getStorage();
    const importedQuotes = await storage.importQuotesFromAPI(allQuotes);
    console.log(`Successfully imported ${importedQuotes.length} quotes`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  seedAllQuotes()
    .then(() => {
      console.log('All quotes seeding completed!');
      process.exit(0);
    })
    .catch((error) => {
